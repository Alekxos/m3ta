...
parser.add_argument('--dataroot', default='/atlas/u/alekxos/datasets')
...
########################################################################
parser.add_argument('--outf', default='/atlas/u/alekxos/domain_adaptation/mettta/ttt_cifar_release/results_meta')
...
net, ext, head, ssh = net.double(), ext.double(), head.double(), ssh.double()
...
for epoch in range(1, args.nepoch+1):
    ...
    for batch_idx, (inputs, labels) in enumerate(trloader):
        ...
        outputs_cls = net(inputs_cls.double())
    ...
    torch.save((all_err_cls, all_err_ssh), path.join(args.outf, 'loss.pth'))
    plot_epochs(all_err_cls, all_err_ssh, path.join(args.outf, 'loss.pdf'))

state = {'err_cls': err_cls, 'err_ssh': err_ssh,
            'net': net.state_dict(), 'head': head.state_dict(),
            'optimizer': optimizer.state_dict()}
torch.save(state, path.join(args.outf, 'ckpt.pth'))
