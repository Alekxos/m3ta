...
def prepare_test_data(args):
	if args.dataset == 'cifar10':
		...
		if not hasattr(args, 'corruption') or args.corruption == 'original':
			...
			teset_raw = np.load(path.join(args.dataroot, 'CIFAR-10-C/%s.npy' %(args.corruption)))
			teset_raw = teset_raw[(args.level-1)*tesize: (args.level-1)*tesize + subsize] # args.level*tesize]
			teset = torchvision.datasets.CIFAR10(root=args.dataroot,
												train=False, download=True, transform=te_transforms)
			...
			teset = CIFAR_New(root=path.join(args.dataroot, 'CIFAR-10.1/datasets'), transform=te_transforms)
			permute = False
		else:
