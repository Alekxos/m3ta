...
def build_model(args):
	...
	net = ResNet(args.depth, args.width, channels=3, classes=classes, norm_layer=norm_layer)
	if not args.no_cuda:
		net.cuda()
	if args.shared == 'none':
		args.shared = None
...
def test(dataloader, model, sslabel=None, cuda=True):
...
def test_grad_corr(dataloader, net, ssh, ext, cuda=True):
	...
	for batch_idx, (inputs, labels) in enumerate(dataloader):
		net.zero_grad()
		ssh.zero_grad()
		inputs_cls, labels_cls = inputs, labels
		if cuda:
		...
		inputs_ssh, labels_ssh = inputs, labels
		if cuda:
